...
import { db } from "@/configs/db";
import { NextResponse } from "next/server";
// 🔥 Remove Inngest import - we're not using it anymore
// import { inngest } from "@/inngest/client";
    ...
    try {
      ...
      const dbResult = await db
        ...
        .values({
          ...
          topic: topic,
          courseLayout: parsedContent,
          status: "Ready", // 🔥 Set status to Ready immediately
        })
        .returning({ resp: STUDY_MATERIAL_TABLE });

      console.log("✅ DB Inserted successfully");

      // 🔥 REMOVE INNGEST TRIGGER - Let users generate notes manually via study-type-content API
      // This way notes will use the same direct generation as other content types
